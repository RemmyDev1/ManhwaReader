...
export const getPythonScript = (settings: ReaderSettings): string => {
  const readZoneTop = settings.readZoneY ?? 20;
  const readZoneHeight = settings.readZoneHeight ?? 55;
  const fuzzyThreshold = settings.fuzzyThreshold ?? 75;
  const omniVoiceUrl = settings.omniVoiceUrl || 'http://127.0.0.1:8001';
  const omniVoiceRefClip = settings.omniVoiceReferenceClip || 'audio.wav';
  const omniVoiceRefText = settings.omniVoiceReferenceText || '';
  const omniVoiceLanguage = settings.omniVoiceLanguage || 'English';
...
DEFAULT_CONFIG = {
    "read_zone_top": ${readZoneTop},
    "read_zone_height": ${readZoneHeight},
    "read_zone_left": 10,
    "read_zone_width": 80,
    "monitor_index": 1,
    ...
    "bubble_settle_sec": 0.85,       # Debounce delay: wait for full speech bubble to scroll into frame
    ...
    "omnivoice_ref_clip": "${omniVoiceRefClip}",
    "omnivoice_ref_text": "${omniVoiceRefText}",
    "omnivoice_language": "${omniVoiceLanguage}",
    "omnivoice_duration_scale": 1.0,
...
MANHWA_CORRECTION_WORDS = COMMON_ENGLISH_WORDS | LONG_VALID_WORDS | {
    ...
    "discovery", "escape", "eternal", "fighting", "future", "hidden",
    "identity", "legend", "legendary", "memory", "opponent", "promise",
    "recognize", "resurrection", "ruins", "servant", "shadow", "strength",
    "survive", "threat", "victory", "weakness", "chapter", "academy",
    "archer", "assassin", "blacksmith", "demon", "dimension", "equipment",
    "experience", "goddess", "heir", "immortal", "ingredient", "merchant",
    "mission", "overlord", "phenomenon", "ranker", "relic", "saint",
    "summoner", "suppress", "transcend", "transformation",
}
...
def correct_manhwa_ocr_word(word: str) -> str:
    ...
    for target in MANHWA_CORRECTION_INDEX:
        if target not in MANHWA_SAFE_CORRECTION_WORDS:
            continue
        if abs(len(target) - len(lower_core)) > 1 or target[0] != lower_core[0]:
            continue
...
def normalize_dialogue_text(text: str, apply_casing: bool = True, apply_repair: bool = True, sanitize_tts: bool = True) -> str:
    ...
    2. Auto-repairs mangled OCR typos and concatenated glowing words.
...
class CosyVoiceTTS(BaseTTS):
    ...
    def prefetch(self, text, speed=1.0, log_fn=None):
        ...
        def worker():
            try:
                wav_file = self._predict_to_wav(clean_text, speed)
                if wav_file and os.path.exists(wav_file):
                    with self._cache_lock:
                        self._prefetch_cache[key] = wav_file
                        while len(self._prefetch_cache) > self._max_prefetch_cache:
                            _, old_path = self._prefetch_cache.popitem()
    ...
    def init_tts(self):
        ...
        if backend == "omnivoice":
            self.tts = OmniVoiceTTS(
                ...
                duration_scale=float(self.config.get("omnivoice_duration_scale", 1.0)),
...
export const requirementsTxt = `customtkinter>=5.2.0
mss>=9.0.1
thefuzz>=0.22.0
pynput>=1.7.6
requests>=2.31.0
...
numpy>=1.24.0
pillow>=10.0.0
winocr>=0.0.1; sys_platform == 'win32'
pywin32>=306; sys_platform == 'win32'
edge-tts>=6.1.0
rapidocr-onnxruntime; python_version < '3.13'
onnxruntime; python_version < '3.13'
`;

export const readmeMd = `# V-Reader Pro: AI Manhwa Screen Reader (v2.6)

V-Reader Pro is a screen reader and auto-scroller built for Manhwa, Manga, and Webtoons.
...
### Features
- **Bubble Settler & Sentence Stabilizer**: Debounces reading until speech bubbles finish scrolling into frame.
- **Natural Dialogue Normalizer & Lexicon Repair**: Auto-corrects comic font OCR errors and converts ALL-CAPS screaming into fluent speech casing.
- **OmniVoice Cloner**: Diffusion-tuned voice cloning with precise duration calculation.
- **Edge-TTS Studio HD**: Crystal-clear Microsoft neural studio voices (Christopher, Guy, Eric, Jenny).
- **1ms Multimedia Smooth Glide**: Sub-notch 60FPS continuous auto-scrolling with zero jitter.
`;

export const runBat = `@echo off
echo ====================================================================
echo Starting V-Reader Pro: AI Screen Reader v2.6...
echo ====================================================================
pip install -r requirements.txt
python main.py
pause
